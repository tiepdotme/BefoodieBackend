...
class CheckoutController extends Controller
{
    	...
    	try {
    	    ...
    	    if($request->payment_type == "1") {
                ...
                try{
                    ...
                    $ordermessage='Đơn hàng có mã số "'.$order_number.'" Các món ăn của bạn đã đặt hàng thành công';
                    ...
                    $title = "Order";
                    $body = 'Đơn hàng có mã số "'.$order_number.'" Các món ăn của bạn đã đặt hàng thành công';
    	    	...
    	    	return response()->json(['status'=>1,'message'=>'Các món ăn của bạn đã đặt hàng thành công'],200);
                ...
                if ($request->order_type == "2") {                    
                    ...
                    if($request->address == ""){
                        return response()->json(["status"=>0,"message"=>"Địa chỉ là bắt buộc"],400);
                    }

                    if($request->lat == ""){
                        return response()->json(["status"=>0,"message"=>"Xin lựa chọn theo gọi ý"],400);
                    }

                    if($request->lang == ""){
                        return response()->json(["status"=>0,"message"=>"Xin chọn theo gợi ý"],400);
                    }

                    if($request->pincode == ""){
                        return response()->json(["status"=>0,"message"=>"mã vùng giao hàng là bắt buộc"],400);
                    }

                    if($request->building == ""){
                        return response()->json(["status"=>0,"message"=>"hẻm, tầng chung cư là bắt buộc"],400);
                    }

                    if($request->landmark == ""){
                        return response()->json(["status"=>0,"message"=>"Chung cư, nhà riêng, hẻm là bắt buộc"],400);
                    }
                ...
                try{
                    $ordermessage='Đơn hàng số "'.$order_number.'" Các món ăn của bạn đã đặt hàng thành công';
                    $email=$getalluses->email;
                    $name=$getalluses->name;
                    $data=['ordermessage'=>$ordermessage,'email'=>$email,'name'=>$name];
                    ...
                    $title = "Đơn hàng";
                    $body = 'Đơn hàng của bạn "'.$order_number.'" đã đặt thành công';
                    $google_api_key = $getdata->firebase; 
                ...
                return response()->json(['status'=>1,'message'=>'đơn hàng đã đặt thành công'],200);
                ...
                if ($request->order_type == "2") {
                    ...
                    if($request->lang == ""){
                        return response()->json(["status"=>0,"message"=>"xin chọn địa chỉ được gợi ý"],400);
                    }

                    if($request->pincode == ""){
                        return response()->json(["status"=>0,"message"=>"Mã vùng bắt buộc phải nhập"],400);
                    }

                    if($request->building == ""){
                        return response()->json(["status"=>0,"message"=>"Số nhà, hẻm, khu phố bắt buộc"],400);
                    }

                    if($request->landmark == ""){
                        return response()->json(["status"=>0,"message"=>"Bắt buộc"],400);
                    }
                ...
                $title = "Đơn hàng";
                $body = 'Đơn hàng mã số "'.$order_number.'" Các món ăn của bạn đã đặt hàng thành công';
                $google_api_key = $getdata->firebase; 
                ...
                return response()->json(['status'=>1,'message'=>'Đơn hàng đã được đặt thành công'],200);
                ...
                if ($request->order_type == "2") {
                    ...
                    if($request->lat == ""){
                        return response()->json(["status"=>0,"message"=>"Xin chọn theo gọi ý"],400);
                    }
                    ...
                    if($request->pincode == ""){
                        return response()->json(["status"=>0,"message"=>"mã vùng là bắt buộc"],400);
                    }

                    if($request->building == ""){
                        return response()->json(["status"=>0,"message"=>"Số nhà, hẻm, nơi dễ nhận hàng"],400);
                    }
                ...
                try{
                    $email=$getalluses->email;
                    $name=$getalluses->name;
                    $ordermessage='Đơn hàng mã số "'.$order_number.'" Các món ăn của bạn đã đặt hàng thành công';
                    $data=['ordermessage'=>$ordermessage,'email'=>$email,'name'=>$name];
    ...
    public function ordercancel(Request $request)
    {
        ...
        {
            return response()->json(['status'=>1,'message'=>'Đơn hàng đã bị hủy bỏ'],200);
        }
        else
        {
    ...
    public function wallet(Request $request)
    {
        ...
        {
            return response()->json(['status'=>1,'message'=>'Transaction list Successful','walletamount'=>number_format($walletamount->wallet ,0),'data'=>$transaction_data],200);
        }   
        else
        {
            return response()->json(['status'=>0,'message'=>'No data found'],200);
    ...
    public function promocodelist()
    {
        ...
        {
            return response()->json(['status'=>0,'message'=>'Không có mã giảm giá'],200);
        }
    }
    ...
    public function promocode(Request $request)
    {
        if($request->offer_code == ""){
            return response()->json(["status"=>0,"message"=>"yêu cầu có mã giảm giá"],400);
        }
        ...
        if ($checkpromo > "0" ) {
            ...
            if($promocode['offer_code']== $request->offer_code) {
                if(!empty($promocode))
                {
                    return response()->json(['status'=>1,'message'=>'Mã khuyến mãi này đã được dùng','data'=>$promocode],200);
                }
            } else {
    ...
    public function checkpincode(Request $request)
    {
        ...
        if(@$pincode['pincode'] == $request->pincode) {
            if(!empty($pincode))
            {
                return response()->json(['status'=>1,'message'=>'mã vùng này có thể giao hàng'],200);
            }
        } else {
            return response()->json(['status'=>0,'message'=>'không thể giao trong mã vùng này'],200);
        }
    }
}
